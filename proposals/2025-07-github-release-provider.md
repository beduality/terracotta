...
Add a `terracotta-provider-github-release` module that publishes Terracotta projects to **GitHub Releases**. It focuses exclusively on GitHub Releases: creating releases, composing release notes, and uploading assets. It depends on the `terracotta-provider-git` module only for Git tags, which a GitHub Release requires. It is designed as a complementary distribution channel, not a Minecraft registry. Operations that do not map to GitHub Releases (gallery, repository lifecycle, repository topics, README/License/CHANGELOG file commits, per-version loaders/game versions) are skipped with a warning rather than emulated poorly.
...
6. Build on the `terracotta-provider-git` module for Git tags rather than duplicating that logic.
...
- **Dependency**: `terracotta-provider-git` (for Git tags only).
...
|---|---|---|
| `CreateProject` | Skip with warning | Repository lifecycle is handled by the `github` provider. |
| `UpdateMetadata` | Skip with warning | Repository description and license metadata are handled by the `github` provider. `LICENSE` file writes are handled by the `git` provider. |
| `UpdateTags` | Skip with warning | Repository topics are handled by the `github` provider. |
| `UpdateDescription` | Skip with warning | `README.md` updates are handled by the `git` provider. The GitHub Release provider does not manage Git content. |
...
Git tag creation is delegated to the `terracotta-provider-git` module. The GitHub Release provider may call the Git provider's API directly or rely on the Git provider being configured in the same Terracotta run.

Authentication via `Authorization: Bearer {token}`.

### 6. Provider Ordering and Dependencies

The `github-release` provider declares a runtime dependency on the `git` provider for tag creation. Terracotta's provider execution engine topologically sorts providers by their declared dependencies and runs them in order (see the [Provider Topological Sort Proposal](2025-07-provider-topological-sort.md)). This guarantees that when both providers are configured, the `git` provider runs first and creates the required tag before the `github-release` provider attempts to create a release.
...
In this configuration, Terracotta runs the `git` provider first to create the tag, then the `github-release` provider to create the release. If `git` is not configured and `github-release` needs a tag that does not already exist, the `github-release` provider fails fast with a clear error telling the user to configure the `git` provider or create the tag manually.

## Migration Path

1. Create `terracotta-provider-github-release` module with build configuration.
2. Add `terracotta-provider-git` as a module dependency for tag creation.
...
5. Declare the `github-release` provider's runtime dependency on the `git` provider for tag creation so the execution engine orders them correctly.
6. Integrate with the Git provider for tag creation.
...
3. **Clear separation**: Git operations are handled by the `git` provider, GitHub repository metadata by the `github` provider, and GitHub Releases by the `github-release` provider.
...
3. **Dependency on the Git provider**: The GitHub Release provider relies on `terracotta-provider-git` for tag creation. If the Git provider is not configured or not available, the GitHub Release provider cannot create tags automatically.
   - **Mitigation**: Document clearly that users must configure the `git` provider when they want GitHub releases with automatic tag creation. Ensure the Git provider SPI is stable.
