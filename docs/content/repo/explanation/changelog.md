...
## How `docs/CHANGELOG.md` differs from module changelogs

Module changelogs follow a promote-on-release cycle: each version bump triggers the release script to parse the `[Unreleased]` section, promote it to a versioned section, and generate a deployment entry. This works because every module publish has a discrete version.

`docs/CHANGELOG.md` works differently. The docs site is versioned by mike major (e.g. `0`), and within a major there are no version bumps — just continuous deploys. The `[Unreleased]` section is therefore a **living changelog for the entire major**, not a staging area that gets promoted on each deploy. It only gets promoted to a versioned section when the mike major is bumped (e.g. `0` → `1`).

This means docs deployment entries cannot be auto-generated from the changelog the way module entries are. Instead, docs deployments are recorded as **manual versionless entries** in `deployments.json` with `"modules": ["docs"]` and no `version` field. When a meaningful docs deploy ships, add an entry by hand with a title and summary that describe what changed in that specific deploy.
...
Unlike module deployments (which are auto-generated by the release script), docs and infrastructure deployment entries are **added manually** to `deployments.json`. For docs deploys, add a versionless entry with `"modules": ["docs"]`, a `title` and `summary` describing what changed in that deploy, and the current `createdAt` timestamp.
